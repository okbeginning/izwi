...
impl VoiceActivityDetector {
    ...
    pub fn process(&mut self, samples: &[f32], is_ai_speaking: bool) -> VadResult {
        ...
        let frame_duration_ms = (self.config.hop_size as f32 / self.config.sample_rate as f32 * 1000.0) as u32;
...
impl FeatureExtractor {
    ...
    pub fn extract(&self, samples: &[f32]) -> Vec<Vec<f32>> {
        ...
        let num_frames = (samples.len().saturating_sub(self.config.frame_size)) / self.config.hop_size + 1;
    ...
    fn process_frame(&self, frame: &[f32]) -> Option<Vec<f32>> {
        ...
        let mel_features: Vec<f32> = self.mel_filterbank.iter()
            .map(|filter| {
                let energy: f32 = filter.iter()
    ...
    fn create_mel_filterbank(fft_size: usize, sample_rate: u32, num_mel_bins: usize) -> Vec<Vec<f32>> {
...
impl LookaheadBuffer {
    ...
    pub fn new(config: SignalFrontendConfig) -> Self {
        let max_samples = (config.sample_rate as usize * config.lookahead_buffer_ms as usize) / 1000;
        Self {
            config,
    ...
    pub fn push(&mut self, samples: &[f32]) {
        ...
        let duration = Duration::from_secs_f64(samples.len() as f64 / self.config.sample_rate as f64);
